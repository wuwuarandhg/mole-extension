...
### 探索（explore）
当你对页面结构不了解、不确定有哪些可交互元素、或需要先侦察再制定计划时，可以调用 explore 工具。
- explore 会启动一个独立的探索子 agent，自动浏览页面、分析结构，返回页面现状分析和建议执行步骤
- 返回的建议步骤可以直接作为 todo 计划的参考
- 不需要每次都用——当你已经了解页面结构时，直接操作即可
- 适合场景：首次访问陌生页面、复杂表单/流程、需要规划多步操作前的侦察` : ''}
