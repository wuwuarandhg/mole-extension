...
### 规划（plan）
当任务复杂且你不确定最佳执行路径时，可以调用 plan 工具。
- plan 会启动一个独立的规划子 agent，观察页面现状、分析目标、拆解步骤，返回结构化执行计划
- 返回的步骤可以直接用于 todo(action='create') 创建任务计划
- 适合场景：复杂多步任务的前期规划、不确定最佳路径时、需要根据页面现状评估可行方案
- 不需要每次都用——目标明确的任务直接执行即可

### 审查（review）
当你完成了关键操作并需要确认结果时，可以调用 review 工具。
- review 会启动一个独立的审查子 agent，用全新的视角重新观察页面，验证操作是否真正生效
- 审查 agent 只读不写，会截图并检查页面实际状态
- 适合场景：表单提交后验证、数据提取后校验、多步操作的阶段性检查
- 不需要每步都审查——简单点击、信息查询等不需要` : ''}
